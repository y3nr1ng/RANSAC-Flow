...
import model.model as model  ## Import rewrite model
...
def computeLossMatchability(network, I, indexRoll, grid, maskMargin, args, SSIM):
    ...
    finalGrad, final = model.predFlowCoarse(
        corr, network["netFlowCoarse"], grid
    )  # final is the flow of image

    match = model.predMatchability(corr, network["netMatch"]) * maskMargin

    # grid_sample to move M(s->t) to the corresponding position to match M(t->s)
    matchCycle = F.grid_sample(match[indexRoll], final) * match

    ## cycle loss on flow

    flowC = F.grid_sample(final[indexRoll].permute(0, 3, 1, 2), final).permute(
        0, 2, 3, 1
    )
    lossCycle = torch.mean(torch.abs(flowC - grid), dim=3).unsqueeze(
        1
    )  ## Dim : N * 1 * W * H
    ...
    lossLr = SSIM(IWarp, I[indexRoll], args.margin)

    ## matchability loss
    lossMatch = torch.sum(torch.abs(1 - matchCycle) * maskMargin) / (
        torch.sum(maskMargin) + 0.001
    )

    # lossGrad = torch.mean(finalGrad)
    loss = lossLr + args.mu_cycle * lossCycle + args.lambda_match * lossMatch

    ## gradient loss:
    lossGrad = torch.tensor(0)
    if args.grad != 0.0:
        # don't know why should times (1 - matchCycle)
        # lossGrad = torch.sum(finalGrad * (1 - matchCycle[:, :, :-1, :-1]) * maskMargin[:, :, :-1, :-1]) / (torch.sum((1 - matchCycle[:, :, :-1, :-1]) * maskMargin[:, :, :-1, :-1]) + 0.001)
        lossGrad = torch.sum(finalGrad * (1 - matchCycle[:, :, :-1, :-1])) / (
            torch.sum((1 - matchCycle[:, :, :-1, :-1])) + 0.001
        )
        loss = loss + args.grad * lossGrad
...
def computeLossNoMatchability(network, I, indexRoll, grid, maskMargin, args, SSIM):
    ...
    f = F.normalize(
        network["netFeatCoarse"](I), p=2, dim=1
    )  # feature extractor for source and target images
    ...
    flowC = F.grid_sample(final[indexRoll].permute(0, 3, 1, 2), final).permute(
        0, 2, 3, 1
    )  # fill value from 1st argument[1] to the position 2nd argument[2,3]
    ## cycle loss on flow
    lossCycle = torch.mean(torch.abs(flowC - grid), dim=3).unsqueeze(
        1
    )  ## Dim : N * 1 * W * H
    ...
    lossGrad = torch.tensor(0)
    if args.grad != 0.0:
        lossGrad = torch.sum(finalGrad * maskMargin[:, :, :-1, :-1]) / (
            torch.sum(maskMargin[:, :, :-1, :-1]) + 0.001
        )
        loss = loss + args.grad * lossGrad

    return lossLr.item(), lossCycle.item(), 0, lossGrad.item(), loss


def computevggLoss(network, I, indexRoll, grid, maskMargin, args, SSIM):
    ...
    IWarp = F.grid_sample(I, final)
    lossLr = SSIM(IWarp, I[indexRoll], args.margin)
    #     lossLr =  model.SSIM(IWarp, I[indexRoll], maskMargin, args.margin, maskMargin, ssim)
    ...
    lossGrad = torch.sum(
        finalGrad * (1 - matchCycle[:, :, :-1, :-1]) * maskMargin[:, :, :-1, :-1]
    ) / (
        torch.sum((1 - matchCycle[:, :, :-1, :-1]) * maskMargin[:, :, :-1, :-1]) + 0.001
    )

    ## perceptual loss
    mse_loss = nn.MSELoss()
    vgg = model.VGG16(requires_grad=False).to(device)

    features_y = vgg(IWarp)
    features_x = vgg(I[indexRoll])

    lossVgg = 0.01 * (
        mse_loss(features_y.relu1_2, features_x.relu1_2)
        + mse_loss(features_y.relu2_2, features_x.relu2_2)
    )

    loss = (
        lossLr
        + args.mu_cycle * lossCycle
        + args.lambda_match * lossMatch
        + args.grad * lossGrad
        + lossVgg
    )
    return (
...
def run(args):
    ...
    network = {
        ...
        "netFlowCoarse": model.NetFlow(args.kernelSize, "netFlowCoarse"),
        "netMatch": model.NetFlow(args.kernelSize, "netMatch"),
    }
    ...
    if args.trainMode == "flow":
        # chain parameters of several networks to optimize jointly
        optimizer = [
            torch.optim.Adam(
                itertools.chain(
                    *[
                        network["netFeatCoarse"].parameters(),
                        network["netCorr"].parameters(),
                        network["netFlowCoarse"].parameters(),
                    ]
                ),
                lr=args.lr,
                betas=(0.5, 0.999),
    ...
    if args.trainMode == "vgg":
        ...
        LossFunction = computevggLoss
        trainModule = ["netFeatCoarse", "netCorr", "netFlowCoarse", "netMatch"]

    ## mask of image. mask on the margin
    # Size: Batchsize * 1 * (imgSize - 2 * margin) * (imgSize - 2 * margin)
    maskMargin = torch.ones(
        args.batchSize * 2,
        1,
    ...
    maskMargin = F.pad(
        maskMargin, (args.margin, args.margin, args.margin, args.margin), "constant", 0
    )
    ...
    ## Train data loader
    trainT = dataloader.trainTransform  # random crop and random flip
    trainLoader = dataloader.TrainDataLoader(
        args.trainImgDir, trainT, args.batchSize, args.imgSize
    )
    ...
    ## define the grid
    gridY = (
        torch.linspace(-1, 1, steps=args.imgSize)
        .view(1, -1, 1, 1)
        .expand(1, args.imgSize, args.imgSize, 1)
    )
    gridX = (
        torch.linspace(-1, 1, steps=args.imgSize)
        .view(1, 1, -1, 1)
    ...
    ## index of batch of images
    index = torch.arange(args.batchSize * 2).cuda()
    indexRoll = torch.roll(index, args.batchSize).cuda()
    ...
    for epoch in range(args.nEpochs):
        ...
        for i, batch in enumerate(trainLoader):
            ...
            # print(f"memory allocated: {a}, reserved: {c}")

            # Forward
            for sub_optimizer in optimizer:
                sub_optimizer.zero_grad()

            lossLr, lossCycle, lossMatch, lossGrad, lossVgg, loss = LossFunction(
                network, I, indexRoll, grid, maskMargin, args, SSIM
            )
            loss.backward()
        ...
        if df is not None:
            precFine = validation.validation(
                df, args.valImgDir, inPklCoarse, network, args.trainMode
            )
        else:
            precFine = np.zeros(8)
        ...
        trainLossVgg = trainLossVgg / len(trainLoader)
        ...
        log_loss = {
            ...
            "trainLossVgg": trainLossVgg,
            "valPrec@8": precFine[4],
        }

        msg = "\n{} Last Update {:d}---> Epoch {:d}, Train Lr Loss : {:.9f}, Train Cycle Loss : {:.9f}, Train Match Loss : {:.9f}, Train Grad Loss : {:.9f}, Train Vgg Loss : {:.9f}, valPrec@8 : {:.9f} (Best {:.9f})-----".format(
            time.ctime(),
            LastUpdate,
            epoch,
            trainLossLr,
        ...
        ## if validation gets better (with annotations), save network parameters
        ...
        ## if no annotations, save network parameters every assigned epoch
        elif df is None and epoch % args.epochSaveModel == args.epochSaveModel - 1:
            outNet = os.path.join(
                args.outDir,
                "checkPoint_Epoch{:d}_Lr{:.3f}_Lf{:.5f}_Lm{:.5f}_Lg{:.5f}_Lv{:.5f}".format(
                    epoch,
                    trainLossLr,
                    trainLossCycle,
                    trainLossMatch,
            ...
            msg = "\n{}\t---> Epoch {:d}. Save model".format(time.ctime(), epoch)
            print(msg)

    ## save network parameters in the last epoch
    if df is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nEpochs", type=int, default=150, help="number of epochs of training"
    )
    ...
    parser.add_argument(
        "--trainImgDir",
        type=str,
        default="../data/MegaDepth/MegaDepth_Train/",
        help="RGB train image directory",
    )
    ...
    parser.add_argument(
        "--lambda-match", type=float, default=0.01, help="matchability loss"
    )

    parser.add_argument(
        "--mu-cycle", type=float, default=1.0, help="weight for cycle distance"
    )

    parser.add_argument(
        "--grad", type=float, default=1, help="weight for cycle distance"
    )

    parser.add_argument(
        "--trainMode",
        choices=["flow", "flow+match", "grad", "vgg"],
        help="choosing module for training...",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=88,
        help="margin size, only training the central patch, in order to get rid of border effect",
    )
    ...
    val.add_argument(
        "--valImgDir",
        type=str,
        default="../data/MegaDepth/Val/img/",
        help="RGB val image directory",
    )

    val.add_argument(
        "--valCSV",
        type=str,
        default="../data/MegaDepth/Val/corr.csv",
        help="val csv containing correspondences",
    )

    val.add_argument(
        "--inPklCoarse",
        type=str,
        default="../data/MegaDepth/Val/coarse.pkl",
        help="input Coarse transformation to make model comparable",
    )
    ...
    noval.add_argument(
        "--inPklCoarse",
        type=str,
        help="input Affine transformation to make model comparable",
    )

    noval.add_argument(
        "--epochSaveModel", type=int, default=5, help="# epochs save one model"
    )
    ...
    if args.trainMode == "grad":
        print(
            "Only learn to smooth flow without touching feature extractor + matchability net..."
        )
    ...
    # torch.cuda.empty_cache() # TEST: empty cache in Cuda
