...
def computeLossMatchability(network, I, indexRoll, grid, maskMargin, args, SSIM):
    ...
    finalGrad, final = model.predFlowCoarse(corr, network['netFlowCoarse'], grid) # final is the flow of image
    ...
    if args.grad != 0.0:
        ...
        lossGrad = torch.sum(finalGrad * (1 - matchCycle[:, :, :-1, :-1]) ) / (torch.sum((1 - matchCycle[:, :, :-1, :-1])) + 0.001)
...
def computeLossNoMatchability(network, I, indexRoll, grid, maskMargin, args, SSIM):
    ...
    flowC = F.grid_sample(final[indexRoll].permute(0, 3, 1, 2), final).permute(0, 2, 3, 1)  # fill value from 1st argument[1] to the position 2nd argument[2,3]
    ## cycle loss on flow
    lossCycle = torch.mean(torch.abs(flowC - grid), dim=3).unsqueeze(1) ## Dim : N * 1 * W * H
...
#     lossLr =  model.SSIM(IWarp, I[indexRoll], maskMargin, args.margin, maskMargin, ssim)
    ...
    lossVgg =  0.01 *(mse_loss(features_y.relu1_2, features_x.relu1_2) + mse_loss(features_y.relu2_2, features_x.relu2_2))

    loss = lossLr  + args.mu_cycle * lossCycle + args.lambda_match * lossMatch + args.grad * lossGrad +  lossVgg
    return lossLr.item(), lossCycle.item(), lossMatch.item(), lossGrad.item(), lossVgg.item(), loss
...
def run(args) :
    ...
    for epoch in range(args.nEpochs):
        ...
        for i, batch in enumerate(trainLoader):
            ...
            lossLr, lossCycle, lossMatch, lossGrad, lossVgg, loss = LossFunction(network, I, indexRoll, grid, maskMargin, args, SSIM)
            loss.backward()
        ...
        log_loss = {'epoch':epoch, 'trainLossLr' : trainLossLr, 'trainLossCycle' : trainLossCycle, 'trainLossMatch':trainLossMatch, 'trainLossGrad':trainLossGrad, 'trainLossVgg': trainLossVgg, 'valPrec@8' : precFine[4]}

        msg = '\n{} Last Update {:d}---> Epoch {:d}, Train Lr Loss : {:.9f}, Train Cycle Loss : {:.9f}, Train Match Loss : {:.9f}, Train Grad Loss : {:.9f}, Train Vgg Loss : {:.9f}, valPrec@8 : {:.9f} (Best {:.9f})-----'.format(time.ctime(), LastUpdate, epoch, trainLossLr, trainLossCycle, trainLossMatch, trainLossGrad, trainLossVgg, precFine[4], bestPrec)
        print (msg)
        ...
        ## if no annotations, save network parameters every assigned epoch
            ...
            outNet = os.path.join(args.outDir, 'checkPoint_Epoch{:d}_Lr{:.3f}_Lf{:.5f}_Lm{:.5f}_Lg{:.5f}_Lv{:.5f}'.format(epoch, trainLossLr, trainLossCycle, trainLossMatch, trainLossGrad, trainLossVgg))
            pth = {}
            for key in list(network.keys()) :
                pth[key] = network[key].state_dict()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trainMode', choices=['flow', 'flow+match', 'grad', 'vgg'], help='choosing module for training...')
    ...
    noval.add_argument('--epochSaveModel', type=int, default = 5, help='# epochs save one model')
