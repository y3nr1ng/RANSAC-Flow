...
def computeGradLossNoMatchability(
    network, I, indexRoll, grid, maskMargin, args, ssim, LrLoss
):
...
def run(args):
    ...
    for epoch in range(args.nEpochs):
        ...
        ## if no annotations, save network parameters every assigned epoch
            ...
            outNet = os.path.join(
                args.outDir,
                "checkPoint_Epoch{:d}_Lr{:.3f}_Lf{:.5f}_Lm{:.5f}_Lg{:.5f}".format(
                    epoch, trainLossLr, trainLossCycle, trainLossMatch, trainLossGrad
                ),
            )
            pth = {}
            for key in list(network.keys()):
